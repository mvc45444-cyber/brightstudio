...
export default function CRMReviewsPage() {
  const crmReviews = reviews.filter(r => r.category === 'CRM Software');

  return (
    <div className="bg-slate-50 min-h-screen pb-20">
      <section className="bg-white border-b border-slate-200 py-16 md:py-24">
        <div className="container mx-auto px-4 md:px-6 text-center max-w-4xl">
           <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-orange-50 text-orange-700 text-sm font-medium mb-6 border border-orange-100">
             <Users className="w-4 h-4" />
             Sales & Customers
           </div>
           <h1 className="text-4xl md:text-5xl font-extrabold text-slate-900 mb-6 tracking-tight">
             Best <span className="text-orange-600">CRM Software</span>
           </h1>
           <p className="text-xl text-slate-600 mb-8 leading-relaxed max-w-2xl mx-auto">
             Customer Relationship Management (CRM) tools are vital for sales growth. We review the giants and the upstarts.
           </p>
        </div>
      </section>
      ...
      <section className="container mx-auto px-4 md:px-6 py-16">
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
           {crmReviews.length > 0 ? crmReviews.map((software) => (
                ...
                <div className="p-6 flex flex-col h-full">
                   <div className="flex justify-between items-start mb-4">
                      <div className="flex items-center gap-3">
                         ...
                         <div>
                            <h3 className="font-bold text-slate-900 text-lg group-hover:text-orange-600 transition-colors">{software.productName}</h3>
